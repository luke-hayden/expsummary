...
listoffilestoblast <- c("1.fa", "2.fa", "3.fa")
blastfilelist <- paste("blast", listoffilestoblast, ".txt", sep="")

for (i in listoffilestoblast){
  
  print(paste("blastp -db parhy1 -query ", i,  " -outfmt '6 qseqid sseqid pident length evalue bitscore stitle' > blast", i , ".txt", sep= ""))
  
}

df <- data.frame(matrix(ncol = 7, nrow = 0) )
colnames(df) <- c("Query", "Target", "Percent_ID", "Ali_len", "E_val", "bitscore", "Hit_name_long" )

givemeres <- 10


for (i in blastfilelist){
  currfile = read.csv(file= i)
  nres <- min(givemeres, nrow(currfile))
  currfile = currfile[1:nres, ]
  df <- rbind(df, currfile)
}




system("blastp -db parhy1 -query junfos.fa -outfmt '6 qseqid sseqid pident length evalue bitscore stitle' > cgblast.txt" )
...
cgTopBlast <- cgTopBlast[order(cgTopBlast$Target),]
cgd$phname <- rownames(cgd)
cgd <- left_join(cgd, cgTopBlast, by=c("phname"="Target"))
...
i <- rownames(resA05) %in% cgd$phname
c <- subset(resA05, i)
cgd$agelog2fc <- c$log2FoldChange
cgd$agepadj <- c$padj


i <- row.names(resR05) %in% cgd$phname
c <- subset(resR05, i)
cgd$reglog2fc <- c$log2FoldChange
cgd$regenpadj <- c$padj

i <- row.names(resONR01) %in% cgd$phname
c <- subset(resONR01, i)
cgd$ONRlog2fc <- c$log2FoldChange

i <- row.names(resOR01) %in% cgd$phname
c <- subset(resOR01, i)
cgd$ORlog2fc <- c$log2FoldChange

i <- row.names(resYNR01) %in% cgd$phname
c <- subset(resYNR01, i)
cgd$YNRlog2fc <- c$log2FoldChange

i <- row.names(resYR01) %in% cgd$phname
c <- subset(resYR01, i)
...
```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
i <- row.names(readsRPKMs) %in% cgd$phname
...
i <- row.names(resRegen) %in% cgd$phname
c <- subset(resRegen, i)
...
```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
vsArthtop <-  subset(vsarthBlast, !(duplicated(vsarthBlast$Query)))
vsArthtop$name <- sub("_phaw.*", "", vsArthtop$Query)

cgd <- left_join(cgd, vsArthtop, by=c("Query"="name"))
cgd$arthit <- sub("^.*\\| ", "", cgd$Hit_name_long.y)

cgd$desc <- paste(cgd$phname, cgd$Query, cgd$arthit, sep="\n" )
...
mcgd <- gather(cgd, key="variable", value="value", c("O1", "O2", "Y1", "Y2", "OR1", "OR2", "YR1", "YR2", "t0_Before", "t1_Amp3", "t2_Amp6", "t3_Reg"))
...
m2cgd <- subset(mcgd, mcgd$variable %in% c("t0_Before", "t1_Amp3", "t2_Amp6", "t3_Reg"))
p=ggplot(m2cgd, aes(x=variable, y=value)) +
  facet_wrap(~ desc,ncol=2,scales = "free_y") +
  xlab("Timepoint")+ylab("Expression (RPKM)") + 
  geom_line(aes(group=desc), color="dark blue", size = 0.8) +
  theme(panel.grid.minor.x=element_blank()) +
  geom_point(color="dark blue", size = 2) +
  theme(legend.position="none") +
  ggtitle("Expression dynamics during regeneration")+ expand_limits(y=0)+
  theme_minimal()+ 
